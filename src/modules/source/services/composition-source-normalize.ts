...
import { propertyName, unwrapExpression } from '@/modules/source/services/compilers/source-expression-compile'
...
function collectPropsEdits(definition: t.ObjectExpression, edits: SourceEdit[]): void {
  ...
  if (!propsProperty || !t.isObjectProperty(propsProperty) || !t.isExpression(propsProperty.value)) {
    return
  }

  const defineProps = unwrapExpression(propsProperty.value)
  if (!t.isCallExpression(defineProps) || !t.isIdentifier(defineProps.callee, { name: 'defineProps' })) {
    return
  }
  ...
  for (const property of props.properties) {
    ...
    if (!typeArgument || !t.isStringLiteral(typeArgument) || !t.isValidIdentifier(typeArgument.value)) {
      continue
    }
    if (typeArgument.start == null || typeArgument.end == null) {
      continue
    }
