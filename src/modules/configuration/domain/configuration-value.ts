...
import type { TypeProgramCatalogEntry, TypeSourceExpression } from '@/modules/source/domain/types/type-source.types'
...
function validateExpression(
  ...
  if (expression.kind === 'reference') {
    ...
    if (identity === 'TriggerSet') {
      if (!Array.isArray(value)) {
        return [error(path, 'Expected TriggerSet array')]
      }
  ...
  if (expression.kind === 'record') {
    if (!isRecord(value)) {
      return [error(path, 'Expected object record')]
    }
  ...
  if (!isRecord(value)) {
    return [error(path, 'Expected object')]
  }
  ...
  for (const field of expression.fields) {
    if (!Object.hasOwn(value, field.key)) {
      if (!field.optional) {
        diagnostics.push(error(`${path}.${field.key}`, 'Required field is missing'))
      }
      continue
    }
    ...
    if (field.array) {
      if (!Array.isArray(fieldValue)) {
        diagnostics.push(error(`${path}.${field.key}`, 'Expected array'))
      }
