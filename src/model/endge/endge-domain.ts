...
export class EndgeDomain extends EndgeModule {
  ...
  compile(): void {
    ...
    // ТРЕЙС СБОРКИ (без lane)
    dbg.startTrace('compile', 'info')

    // КОМПИЛЯЦИЯ НАСТРОЕК
    dbg.startSpan(ENDGE_LOG_LANES.ACTIONS, 'compile', 'info')
    dbg.info('Начата компиляция действий')
    try {
      this._actionsById.forEach(action => action.compile())
      dbg.info('Компиляция действий успешно завершена', {
        icon: 'ti ti-check text-xl',
      })
      dbg.endSpan('info')
    }
    catch (e: any) {
      dbg.error(`Ошибка компиляции действий: ${e?.message ?? e}`, { error: e })
    ...
    // КОМПИЛЯЦИЯ ТИПОВ
    dbg.startSpan(ENDGE_LOG_LANES.SETTINGS, 'compile', 'info')
    dbg.info('Начата компиляция настроек')
    try {
      this._settingsById.forEach(setting => setting.compile())
      dbg.info('Компиляция настроек успешно завершена', {
        icon: 'ti ti-check text-xl',
      })
      dbg.endSpan('info')
    }
    catch (e: any) {
      dbg.error(`Ошибка компиляции настроек: ${e?.message ?? e}`, { error: e })
      dbg.error('action: alert', { icon: 'ti ti-alert-triangle text-xl' })
      dbg.endSpan('error')
      dbg.endTrace('error', { failedAt: 'types' })
      return
    }

    // КОМПОНЕНТЫ: топосорт
    const edges: Array<[string | number, string | number]> = []
    for (const component of this._componentsById.values()) {
      const deps = component.getDependencyComponentIds()
      for (const depId of deps) edges.push([depId, component.id])
    }

    // КОМПИЛЯЦИЯ КОМПОНЕНТОВ
    dbg.startSpan(ENDGE_LOG_LANES.COMPONENTS, 'compile', 'info')
    dbg.info('Начата компиляция компонентов')

    // Отдельный span на топосортировку, чтобы события были внутри контекста
    dbg.startSpan(ENDGE_LOG_LANES.COMPONENTS, 'toposort', 'info')
    let orderedComponentIds: (string | number)[] = []
    try {
      orderedComponentIds = toposort(edges)
      dbg.endSpan('info')
    }
    catch (e: any) {
      dbg.error(`Ошибка топосортировки компонентов: ${e?.message ?? e}`, {
        error: e,
      })
      dbg.error('Обнаружены циклические зависимости компонентов')
      dbg.endSpan('error')
      dbg.endTrace('error', { failedAt: 'components-toposort' })
      throw new Error('Ошибка: Обнаружены циклические зависимости компонентов')
    }
    const orderedSet = new Set<string | number>(orderedComponentIds)
    for (const componentId of this._componentsById.keys()) {
      if (!orderedSet.has(componentId))
        orderedComponentIds.push(componentId)
    }

    try {
      for (const componentId of orderedComponentIds) {
        const component = this._componentsById.get(componentId)
        if (component)
          component.compile()
      }
      dbg.info('Компиляция компонентов успешно завершена', {
        icon: 'ti ti-check text-xl',
      })
      dbg.endSpan('info')
    }
      ...
      console.error(e)
      dbg.error(`Ошибка компиляции компонентов: ${e?.message ?? e}`, {
        error: e,
      })
      dbg.error('action: alert', { icon: 'ti ti-alert-triangle text-xl' })
      dbg.endSpan('error')
    ...
    // КОМПИЛЯЦИЯ ЗАПРОСОВ
    dbg.startSpan(ENDGE_LOG_LANES.QUERIES, 'compile', 'info')
    dbg.info('Начата компиляция запросов')
    try {
      this._queriesById.forEach(query => query.compile())
      dbg.info('Компиляция запросов успешно завершена', {
        icon: 'ti ti-check text-xl',
      })
      dbg.endSpan('info')
    }
    catch (e: any) {
      dbg.error(`Ошибка компиляции запросов: ${e?.message ?? e}`, { error: e })
      dbg.error('action: alert', { icon: 'ti ti-alert-triangle text-xl' })
      dbg.endSpan('error')
      dbg.endTrace('error')
      return
    ...
    dbg.startSpan(ENDGE_LOG_LANES.SCENARIOS, 'compile', 'info')
    dbg.info('Начата компиляция сценариев')
    try {
      this._scenariosById.forEach(scenario => scenario.compile())
      dbg.info('Компиляция сценариев успешно завершена', {
        icon: 'ti ti-check text-xl',
      })
      dbg.endSpan('info')
    }
    catch (e: any) {
      dbg.error(`Ошибка компиляции сценариев: ${e?.message ?? e}`, { error: e })
      dbg.error('action: alert', { icon: 'ti ti-alert-triangle text-xl' })
      dbg.endSpan('error')
      dbg.endTrace('error')
      return
