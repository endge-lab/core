...
export class StoreRuntimeHost extends RuntimeHostBase<'store', RuntimeHostContext<'store'>, StoreSourceArtifact> {
  ...
  private _validateMutationPlans(plans: StoreMutationPlan[]): void {
    ...
    // Existing scalar targets cannot change the path structure. The live path
    // lookup also reuses its selector index for large collections.
    if (plans.every((plan) => {
      if (plan.strategy !== 'set') {
        return false
      }
      if (plan.plane === 'meta' && !String(plan.namespace ?? '').trim()) {
        return false
      }
      if (plan.plane !== 'meta' && plan.value !== null && typeof plan.value === 'object') {
        return false
      }
      const path = appendRawStorePath(this.getDataPath(), plan.path)
      return Raph.has(path, plan.vars ? { vars: plan.vars } : undefined)
    })) {
      return
    }
    const projected = new DefaultDataAdapter((Raph.get('') ?? {}) as Record<string, unknown>).fork()
    for (const plan of plans) {
      const path = appendRawStorePath(this.getDataPath(), plan.path)
      const options = plan.vars ? { vars: plan.vars } : undefined
      if (plan.plane === 'meta') {
