...
import type { ComputationResource } from '@/features/core/modules/domain/types/computation/computation-runtime.types'
...
> {
  ...
  private _makeCollectionPatch(
    ...
    for (const event of ctx.events) {
      if (!isCollectionEventPath(sourcePath, event.path)) {
        return null
      }
      const selector = DataPath.from(event.path).segments()[sourceSegmentCount]
      if (selector?.kind !== 'index' && selector?.kind !== 'selector') {
        return null
      }
      const selectorKey = selector.kind === 'selector' ? selector.key : null
      const selectorValue = selector.kind === 'selector' ? selector.value : selector.index
      const groupKey = selectorKey
        ? `key:${selectorKey}:${typeof selectorValue}:${String(selectorValue)}`
        : `index:${String(selectorValue)}`
      const group = groups.get(groupKey) ?? { events: [] as Array<(typeof ctx.events)[number]>, selectorKey, selectorValue }
    ...
    const items = [...groups.values()].map((group) => {
      ...
      return {
        ...
        changedPaths: group.events
          .map(event => this._extractChangedPath(sourcePath, event.path))
