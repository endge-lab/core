...
export class FilterRuntimeHost extends RuntimeHostBase<'filter', RuntimeHostContext<'filter'>, FilterProgramPayload> {
  ...
  public static createRuntime(input: {
    ...
    const explicitArtifact = input.meta?.artifact as import('@/features/core/modules/program/domain/types/program.types').ProgramArtifact<FilterProgramPayload> | undefined
    const artifactReader: RuntimeArtifactReader = explicitArtifact
      ? {
          getArtifact: <TPayload>() => explicitArtifact as unknown as import('@/features/core/modules/program/domain/types/program.types').ProgramArtifact<TPayload>,
        }
      : input.artifacts
