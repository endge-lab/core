...
} from '@/features/core/modules/styles/domain/types/style.types'
...
function nthMatches(expression: string, index: number): boolean {
  ...
  if (/^[+-]?\d+$/.test(normalized)) {
    return index === Number(normalized)
  }
...
function compoundMatches(compound: EndgeStyleCompoundSelector, node: EndgeStyleMatchNode): boolean {
  ...
  return compound.pseudos.every((pseudo) => {
    ...
    if (pseudo.name === 'last-child') {
      return node.index === node.siblingCount
    }
    if (pseudo.name === 'nth-child') {
      return nthMatches(pseudo.expression, node.index)
    }
    if (pseudo.name === 'component') {
      return node.component === pseudo.value
    }
    if (pseudo.name === 'identity') {
      return node.identity === pseudo.value
    }
    if (pseudo.name === 'state') {
      return node.states.has(pseudo.value)
    }
    if (pseudo.name === 'part') {
      return node.parts.has(pseudo.value)
    }
    if (pseudo.name === 'not') {
...
export function evaluateEndgeStyleSupport(
  ...
  if (condition.type === 'renderer') {
    return condition.renderer === target.renderer
  }
  if (condition.type === 'capability') {
    return new Set(target.capabilities ?? []).has(condition.capability)
  }
  if (condition.type === 'not') {
    return !evaluateEndgeStyleSupport(condition.operand, target)
  }
  if (condition.type === 'and') {
    return condition.operands.every(operand => evaluateEndgeStyleSupport(operand, target))
  }
...
export function matchEndgeStyleRule(
  ...
  if (rule.theme && rule.theme !== theme) {
    return []
  }
