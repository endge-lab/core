...
- Встроенная функция `t('resource:key.path', fallback?)` является исключением: она читает накопленный i18n catalog текущего Composition scope.
- Встроенная функция `vocab('alias', mapping?)` является вторым исключением: она читает Vocab alias ближайшего Composition scope.
...
<Text :tooltip="t('schedule:refreshHint', 'Обновить')">...</Text>
```

Имя до `:` — public resource alias, объявленный как `i18n(identity)` в Composition. Если перевод и fallback отсутствуют, результат имеет вид `{{schedule:columns.status}}`. Смена locale в `Endge.context` инвалидирует application scope и запускает повторный render.

### Справочники

SFC не принимает справочники через `defineProps` и не загружает их самостоятельно.
Composition объявляет физические Vocab dependencies, а имя поля `data` становится
публичным alias для дочерних компонентов:
...
Компонент читает alias через встроенную функцию `vocab()`:
...
Первый аргумент всегда является статическим публичным alias, а не Vocab identity.
Второй аргумент, если указан, является статическим object literal с обязательными
строковыми `valuePath` и `labelPath`. Без mapping используются пути `value` и
`label`, то есть справочник уже должен содержать `SourceFieldOption[]`.

Runtime получает накопленный catalog от ближайшего Composition scope. Локальный
alias вложенного scope перекрывает одноимённый alias предка. Catalog хранит
реактивный Raph path, поэтому обновление `Endge.vocabs` повторно рендерит SFC без
замены props. Если alias не предоставлен runtime-деревом, это runtime error;
поиск по глобальному cache или физической identity не выполняется.
